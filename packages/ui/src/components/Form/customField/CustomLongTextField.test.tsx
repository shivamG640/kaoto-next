...
describe('CustomLongTextField', () => {
  ...
  it('should display the security properties', async () => {
    render(
      <AutoField.componentDetectorContext.Provider value={CustomAutoFieldDetector}>
        <AutoForm schema={schemaBridge!}>
          <CustomNestField name="parameters" />
    ...
    const buttonElement = screen.getByRole('button', { name: 'Security properties' });
    await act(async () => {
      fireEvent.click(buttonElement);
    });
    const inputSaslJaasConfigElement = screen
      .getAllByRole('textbox')
      .filter((textbox) => textbox.getAttribute('data-testid') === 'long-text-field');
    expect(inputSaslJaasConfigElement).toHaveLength(2);
  });
});
