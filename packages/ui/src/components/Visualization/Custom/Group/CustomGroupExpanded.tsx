...
const groupDropTargetSpec: DropTargetSpec<GraphElement, unknown, object, GraphElementProps> = {
  ...
  canDrop: (item, _monitor, props) => {
    const draggedNode = (item as Node).getData().vizNode;
    const targetGroup = (props.element as Node).getData().vizNode;
    // dragged node cant be dropped on the same level i.e. at its oriinal place
    if (!!props && draggedNode.parentNode.id !== targetGroup.id) {
      // dragged node can go only to root group incase on nested group
      if (draggedNode.parentNode.id === targetGroup.parentNode?.id) {
        return targetGroup.data.processorName !== 'choice';
      }
    }
