...
describe('CanvasFormTabs', () => {
  ...
  describe('should show the User-updated field under the modified tab', () => {
    ...
    it('dataformat field', async () => {
      ...
      expect(screen.queryByRole('button', { name: 'Menu toggle' })).toBeNull();

      act(() => {
        fireEvent.click(defaultTab);
      });

      const dataformatDefaultTabButton = screen.getAllByRole('button', { name: 'Menu toggle' });
      await act(async () => {
      ...
      expect(screen.queryByRole('button', { name: 'Menu toggle' })).toBeInTheDocument();
    });
