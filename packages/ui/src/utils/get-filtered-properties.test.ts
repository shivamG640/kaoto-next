...
describe('getFilteredProperties()', () => {
  ...
  } as unknown as KaotoSchemaDefinition['schema']['properties'];

  it('should return only the filtered properties', () => {
    const filteredSchema = getFilteredProperties(schemaObject, 'des');
    expect(filteredSchema).toMatchSnapshot();
  });

  it('should return only the un-omitted properties', () => {
    const filteredSchema = getFilteredProperties(schemaObject, '', ['get', 'post', 'put', 'delete', 'patch', 'patch']);
    expect(filteredSchema).toMatchSnapshot();
  });
});
